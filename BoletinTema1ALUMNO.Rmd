...
rownames(commonAnimals_1)

commonAnimals_2 <- intersect(Animals2, mammals)
rownames(commonAnimals_2)
```
...
diferentes_m_animales <- setdiff(rownames(mammals), rownames(Animals2))
diferentes_m_animales

#Animales en Animals2 pero no en mammals
diferentes_a_animales <- setdiff(rownames(Animals2), rownames(mammals))
diferentes_a_animales
```

f.  Determina las diferencia entre los animales que no aparecen en ambos conjuntos.
```{r}
#No hay ninguna diferencia entre los animales, solo que en cada data frame lo llaman de una forma
```
...
```{r}
library()
#Tenemos instaladas 1037 librerias
```
...
#Vemos que no existe una relacion lineal
```
